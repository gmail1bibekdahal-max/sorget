...
export default function SignupPage() {
  ...
  const handleGoogle = async () => {
    track({
      ...
      properties: { page: "signup", auth_provider: "google" },
    });
    saveLead({
      step_reached: "signup_google",
      raw_data: { method: "google", source: "signup_page" },
    });
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: typeof window !== "undefined" ? `${window.location.origin}/signup/setup` : undefined,
        },
      });
      if (error) {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    saveLead({
      ...
      email: email,
      step_reached: "signup",
      raw_data: { full_name: fullName, email: email, has_password: Boolean(password) },
    });
    ...
    try {
      ...
      if (error) {
        console.warn("[Auth] Supabase signup response:", error.message);
      }
    ...
    <div className={styles.page}>
      <div className={styles.formSide}>
        <div className={styles.formBox}>
          ...
          <form onSubmit={handleCreateAccount}>
            <div className={styles.field}>
              ...
              <input
                ...
                onFocus={() => track("focus_field", { page: "signup", field: "full_name" })}
                onBlur={(e) => {
                  track("fill_field", { page: "signup", field: "full_name", value: e.target.value, filled: !!e.target.value });
                }}
              />
            </div>
          ...
          <p style={{ fontSize: "12px", color: "var(--olvy-grey)", textAlign: "center", marginTop: "16px" }}>
            By signing up, you agree to our{" "}
            <a href="#" style={{ color: "var(--olvy-pink)" }} onClick={() => track({ event_name: "click_terms", target_text: "Terms", properties: { page: "signup" } })}>Terms</a>{" "}
            and{" "}
            <a href="#" style={{ color: "var(--olvy-pink)" }} onClick={() => track({ event_name: "click_privacy", target_text: "Privacy Policy", properties: { page: "signup" } })}>Privacy Policy</a>.
          </p>
        </div>
      </div>
